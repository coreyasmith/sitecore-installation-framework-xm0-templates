...
$SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.4.0 rev. * (XM) (OnPrem)_cm.scwdp.zip").FullName
...
if ($Update) {
    $SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.4.* PRE (XM) (OnPrem)_cm.cumulative.delta.scwdp.zip").FullName
