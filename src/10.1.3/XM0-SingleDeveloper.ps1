...
$SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.1.3 rev. * (XM) (OnPrem)_cm.scwdp.zip").FullName
...
$HeadlessServicesCumulativeHotfixLayoutServicePackage = (Get-ChildItem "$SCInstallRoot\packages\SC Hotfix-549890-* Layout Service *.zip").FullName
# The path to the Headless Services GraphQL cumulative hotfix Package to Deploy.
$HeadlessServicesCumulativeHotfixGraphQLPackage = (Get-ChildItem "$SCInstallRoot\packages\SC Hotfix-569574-* GraphQL *.zip").FullName

if ($Update) {
    $SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.1.* PRE (XM) (OnPrem)_cm.delta.scwdp.zip").FullName
...
# Apply the Sitecore cumulative update instructions.
$sitecoreUpdateParams = @{
    Path = "$SCInstallRoot\templates\sitecore-update.json"
    SiteName = $SitecoreSiteName
}
...
if (!$Uninstall) {
    if (!$Update) {
        ...
        Install-SitecoreConfiguration @singleDeveloperParams *>&1 | Tee-Object (Join-Path $LogsDirectory 'XM0-SingleDeveloper-Update.log')
        Install-SitecoreConfiguration @sitecoreUpdateParams *>&1 | Tee-Object (Join-Path $LogsDirectory 'Sitecore-Update.log')
