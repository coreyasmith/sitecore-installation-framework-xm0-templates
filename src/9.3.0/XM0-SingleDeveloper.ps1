...
$SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 9.3.0 rev. * (XM) (OnPrem)_cm.scwdp.zip").FullName
...
$SitecoreCumulativeHotfixPackage = (Get-ChildItem "$SCInstallRoot\packages\SC Hotfix PDXP-7469-*.zip").FullName
# The path to the SXA cumulative hotfix Package to Deploy.
$SXACumulativeHotfixPackage = (Get-ChildItem "$SCInstallRoot\packages\SC Hotfix SXA-8487-*.zip").FullName
...
$sc2025_003Params = @{
    ...
    # The Sitecore 9.3 cumulative hotfix requires that 628372 not be installed.
    ExcludeFilter = "*628372*"
}
