...
$SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.2.2 rev. * (XM) (OnPrem)_cm.scwdp.zip").FullName
...
if ($Update) {
    $SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.2.* PRE (XM) (OnPrem)_cm.delta.scwdp.zip").FullName
...
# Install SPE via combined partials file. The SXA cumulative hotfix ships its own copy of the SPE
# item resource files, so SPE is installed again after it.
$speParams = @{
