...
$SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.3.3 rev. * (XM) (OnPrem)_cm.scwdp.zip").FullName
...
if ($Update) {
    $SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.3.* PRE (XM) (OnPrem)_cm.delta.scwdp.zip").FullName
