...
$SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.2.1 rev. * (XM) (OnPrem)_cm.scwdp.zip").FullName
...
if ($Update) {
    $SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.2.* PRE (XM) (OnPrem)_cm.cumulative.delta.scwdp.zip").FullName
