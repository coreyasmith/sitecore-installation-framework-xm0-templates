...
$SolrRoot = "C:\Solr\Solr-9.8.1"
# The Name of the Solr Service.
$SolrService = "Solr-9.8.1"
...
$SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.4.1 rev. * (XM) (OnPrem)_cm.scwdp.zip").FullName
...
# The path to the Headless Services JSS cumulative hotfix Package to Deploy.
$HeadlessServicesCumulativeHotfixJSSPackage = (Get-ChildItem "$SCInstallRoot\packages\SC Hotfix*JSS*.zip").FullName
...
if ($Update) {
    $SitecorePackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore 10.4.* PRE (XM) (OnPrem)_cm.delta.scwdp.zip").FullName
    $IdentityServerPackage = (Get-ChildItem "$SCInstallRoot\packages\Sitecore.IdentityServer * rev. * (OnPrem)_identityserver.delta.scwdp.zip").FullName
}
...
$singleDeveloperParams = @{
    ...
    IdentityServerEncryptSecurityDbConnection = $SqlEncrypt
    IdentityServerTrustServerCertificateSecurityDbConnection = $SqlTrustServerCertificate
...
# Install the Headless Services JSS cumulative hotfix.
$headlessServicesCumulativeHotfixJSSParams = @{
    Path = "$SCInstallRoot\templates\sitecore-module-hotfix.json"
    Package = $HeadlessServicesCumulativeHotfixJSSPackage
    SiteName = $SitecoreSiteName
    SatToolsPath = $AzureToolkitToolsPath
}
...
$headlessServicesCumulativeHotfixLayoutServiceParams = @{
    ...
    SiteName = $SitecoreSiteName
    SatToolsPath = $AzureToolkitToolsPath
    DisableFiles = @("App_Config\Sitecore\LayoutService\Sitecore.LayoutService.Analytics.config")
}
...
if (!$Uninstall) {
    ...
    if ($Update) {
        ...
        Install-SitecoreConfiguration @headlessServicesCumulativeHotfixJSSParams *>&1 | Tee-Object (Join-Path $LogsDirectory 'Sitecore.HeadlessServices-CumulativeHotfix-JSS.log')
